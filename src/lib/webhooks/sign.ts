...
//
// Every delivery carries an `X-Erascrm-Signature` header so receivers
// can verify the request really came from erascrm and wasn't tampered
// with or replayed. The scheme is Stripe-style:
//
//   X-Erascrm-Signature: t=<unix_seconds>,v1=<hex HMAC-SHA256>
//
...
/**
 * Build the `X-Erascrm-Signature` header value for `rawBody`, signed
...
/**
 * Verify a signature header. Exposed so an erascrm-to-erascrm
 * integration (or a test) can validate deliveries; receivers in other stacks
