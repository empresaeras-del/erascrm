...
describe('hashApiKey', () => {
  it('is deterministic', () => {
    expect(hashApiKey('erascrm_live_abc')).toBe(hashApiKey('erascrm_live_abc'));
  });

  it('differs for different inputs', () => {
    expect(hashApiKey('erascrm_live_abc')).not.toBe(hashApiKey('erascrm_live_abd'));
  });
});
...
describe('timingSafeHexEqual', () => {
  it('is true for identical digests', () => {
    const h = hashApiKey('erascrm_live_xyz');
