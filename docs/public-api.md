...
The public API lets you drive your erascrm instance from your own
...
```
Authorization: Bearer erascrm_live_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
3. Copy the key. **The full key is shown exactly once.** erascrm
...
curl https://your-crm.example.com/api/v1/me \
  -H "Authorization: Bearer erascrm_live_xxx"
```
...
Rather than polling, register an endpoint and erascrm will POST to it when
...
- `POST /api/v1/webhooks` — register `{ "url": "https://…", "events": ["message.received"] }`. `url` must be `https://`. **The response includes `secret` exactly once** — store it to verify signatures; erascrm keeps only an encrypted copy.
...
curl -X POST https://your-crm.example.com/api/v1/webhooks \
  -H "Authorization: Bearer erascrm_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "url": "https://example.com/hooks/erascrm", "events": ["message.received"] }'
...
Headers: `X-Erascrm-Event`, `X-Erascrm-Webhook-Id`, and `X-Erascrm-Signature`.

### Verifying the signature

`X-Erascrm-Signature: t=<unix_seconds>,v1=<hex>` where `v1 =
...
covers messages erascrm stores (inbox + API sends), not broadcast-only
